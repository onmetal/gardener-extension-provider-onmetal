...
func (a *actuator) Reconcile(ctx context.Context, log logr.Logger, backupBucket *extensionsv1alpha1.BackupBucket) error {
	...
	onmetalClient, namespace, err := onmetal.GetOnmetalClientAndNamespaceFromCloudProviderSecret(ctx, a.Client(), backupBucket.Namespace)
	if err != nil {
...
// returns secretRef from onmetal backupBukcet access
...
// updates backupBucket status with secretRef
